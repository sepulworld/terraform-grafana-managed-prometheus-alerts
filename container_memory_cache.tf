resource "grafana_rule_group" "k8s_container_memory_cache" {
  ...
  rule {
    name      = "ContainerMemoryCacheRecordingRule"
    condition = "A" # Reference to the query node

    # Recording Rule
    record {
      from   = "A" # Refers to the data query
      metric = "node_namespace_pod_container:container_memory_cache"
    }
    ...
    annotations = {
      description = "Record container memory cache usage by cluster, namespace, and pod."
    }
